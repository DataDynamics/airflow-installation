...
# --- Redis Sentinel (고가용 브로커) -----------------------------------------
# true 면 브로커를 단일 Redis 대신 Sentinel 이 관리하는 master/replica 로 구성한다.
# 각 redis 노드에 redis-sentinel 을 함께 띄우고, 브로커 URL 을 sentinel:// 로 생성한다.
# (master 장애 시 sentinel 이 replica 를 자동 승격 → 브로커 무중단)
export REDIS_SENTINEL_ENABLED="${REDIS_SENTINEL_ENABLED:-false}"
export REDIS_MASTER_NAME="${REDIS_MASTER_NAME:-airflow-redis}"        # sentinel 이 감시하는 master 이름
export REDIS_SENTINEL_PORT="${REDIS_SENTINEL_PORT:-26379}"
# sentinel 호스트 목록(공백/콤마 구분). HA 는 홀수 3대 이상 권장. 비우면 CONTROL_IP 단독.
export REDIS_SENTINEL_HOSTS="${REDIS_SENTINEL_HOSTS:-${CONTROL_IP}}"
# 이 노드 redis 역할: master | replica  (INSTALL_REDIS=true 인 노드에만 의미)
export REDIS_ROLE="${REDIS_ROLE:-master}"
# replica 가 복제할 master 주소(부트스트랩 초기값; 이후 sentinel 이 자동 관리)
export REDIS_MASTER_HOST="${REDIS_MASTER_HOST:-${CONTROL_IP}}"
export REDIS_SENTINEL_QUORUM="${REDIS_SENTINEL_QUORUM:-2}"
export REDIS_SENTINEL_DOWN_AFTER_MS="${REDIS_SENTINEL_DOWN_AFTER_MS:-5000}"
export REDIS_SENTINEL_FAILOVER_TIMEOUT_MS="${REDIS_SENTINEL_FAILOVER_TIMEOUT_MS:-10000}"
...
# --- 파생: Celery 브로커 URL (sentinel / 단일 redis, 인증 유무 반영) ---
# Sentinel 모드: sentinel://[:pw@]host:port;... (세미콜론 구분, 전 sentinel 나열).
#   URL 의 비밀번호는 redis 데이터 노드 접속용(masterauth/requirepass). sentinel 자체는
#   무인증으로 두므로 sentinel_kwargs 는 불필요. master_name 은 05 에서 airflow.cfg 의
#   [celery_broker_transport_options] 에 기록한다(환경변수로는 kombu 에 전달되지 않음).
if [ "${REDIS_SENTINEL_ENABLED}" = "true" ]; then
  _authp=""; [ -n "${REDIS_PASSWORD}" ] && _authp=":$(urlenc "${REDIS_PASSWORD}")@"
  _burl=""
  for _sh in ${REDIS_SENTINEL_HOSTS//,/ }; do
    [ -n "${_burl}" ] && _burl="${_burl};"
    _burl="${_burl}sentinel://${_authp}${_sh}:${REDIS_SENTINEL_PORT}"
  done
  export BROKER_URL="${_burl}"
